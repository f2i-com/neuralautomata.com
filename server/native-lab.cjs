...
function createLab({ root, port, executeFile = execFile, spawnProcess = spawn }) {
  ...
  const lab = path.resolve(process.env.NCA_LAB_DIR || path.join(root, 'research', 'fast_memory_language'));
  ...
  async function handle(req, res, url) {
    ...
    try {
      if (url.pathname === '/api/nca/status' && req.method === 'GET') {
        ...
        if (req.headers['x-nca-token'] !== token || String(req.headers['content-type'] || '').split(';', 1)[0].trim().toLowerCase() !== 'application/json') {
