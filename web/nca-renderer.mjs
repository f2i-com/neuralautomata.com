...
export function createStateRenderer(canvas) {
    ...
    if(hasData==1){
      ...
      if(ring==1){
        ...
        for(int i=0;i<8;i++){
          ...
          float d=length(p-center);float rms=rowRms[i];
  ...
  let lost = false, textureWidth = 0, textureHeight = 0;
  ...
  function draw(frame, memory = true) {
    ...
    if (textureWidth === data.width && textureHeight === data.height) {
      gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, data.width, data.height, gl.RED, gl.FLOAT, values);
    } else {
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.R32F, data.width, data.height, 0, gl.RED, gl.FLOAT, values);
      textureWidth = data.width; textureHeight = data.height;
    }
    // Eight reductions per snapshot, rather than per pixel in the ring shader.
    const rms = new Float32Array(8);
    if (memory) for (let row = 0; row < Math.min(8, data.height); row++) {
      let sum = 0;
      for (let col = 0; col < data.width; col++) sum += values[row * data.width + col] ** 2;
      rms[row] = Math.sqrt(sum / data.width);
    }
    gl.uniform1fv(uniform('rowRms[0]'), rms);
